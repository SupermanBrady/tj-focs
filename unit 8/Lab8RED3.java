...
class TrianglePanel extends JPanel
{
   ...
   public void animate()
   {
      ...
      if (enemy != null)
      {
         if (rocket != null && enemy.overlap(rocket.getX(), rocket.getY(), rocket.getW()))
         {
          enemy = null;
         }
         else
...
class Rectangle
{
   ...
   public void move()
   {
      ...
      else if (X + width >= TrianglePanel.FRAME)
   ...
   public void draw(Graphics myBuffer)
   {
      ...
      myBuffer.fillRect(X, Y, width, height);
   }
   
   public boolean overlap(int x, int y, int w)
   {
      return (((x+w >= X && x+w <= X+width)||(x >= X && x <= X+width))&&((y <= Y+height && y >= Y)||(y+w <= Y+height && y+w >= Y)));
   }
}